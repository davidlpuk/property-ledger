...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["text/csv", "application/pdf", "text/plain"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for bank-files" ON storage.objects FOR SELECT USING (bucket_id = 'bank-files');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for bank-files" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'bank-files');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for bank-files" ON storage.objects FOR UPDATE USING (bucket_id = 'bank-files');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for bank-files" ON storage.objects FOR DELETE USING (bucket_id = 'bank-files');`
    ];
